...
export const EditCompanyPage = () => {
  ...
  const { data, isLoading: loadingCompany } = useQuery({
    queryKey: ["myCompany"],
  ...
  const mutation = useMutation({
    mutationFn: (formData: {
      ...
      website?: string;
      benefits?: string[];
    }) => companyService.update(data!.data._id, formData),
    onSuccess: () => {
  ...
  if (loadingCompany) return <p className="text-center text-gray-500 dark:text-gray-400">{t("company.loading")}</p>;
  if (!data?.data) return <p className="text-center text-red-500">{t("company.notFound")}</p>;

  return (
    <div className="mx-auto max-w-2xl">
      <Link to="/recruiter/jobs" className="mb-4 inline-block text-sm text-primary-500 hover:underline">
        &larr; {t("company.backToJobs")}
      </Link>

      <h1 className="mb-6 text-2xl font-bold">{t("company.editTitle")}</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        >
          {mutation.isPending ? t("company.saving") : t("company.saveChanges")}
        </button>
      </form>
    </div>
  );
